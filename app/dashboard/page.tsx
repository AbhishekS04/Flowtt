...
export default async function DashboardPage() {
  ...
  try {
    ...
    try {
      ...
      console.warn("SIP sync warning:", e);
    }
    ...
    const [monthExpenses, catBudgets, customCats, monthIncomes, allExpenses, allIncomes, userSips, userGoals, userRecharges] = await withRetry(() =>
      Promise.all([
  ...
  const onlineBalance = parseFloat(user.initialOnlineBalance || "0") + onlineIncomes - onlineExpenses;
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center p-4">
        <div className="bg-card border border-border rounded-3xl p-8 max-w-md w-full text-center space-y-4">
          <div className="w-12 h-12 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center mx-auto text-xl">
            ⚡
          </div>
          <h2 className="text-text-primary font-bold text-xl tracking-tighter">
            Connection Interrupted
          </h2>
          <p className="text-text-muted text-xs leading-relaxed">
            The database connection had a momentary network timeout. Please click below to refresh and reconnect.
          </p>
          <a
            href="/dashboard"
            className="inline-block w-full py-2.5 px-4 rounded-xl bg-text-primary text-bg font-black text-xs uppercase tracking-widest hover:opacity-90 transition-opacity"
          >
            Reconnect & Reload
          </a>
        </div>
